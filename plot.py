...
if __name__ == "__main__":
    ...
    EMPIRICAL_ELEMENTWISE = df_cupy["flops"].max()

    plt.figure(figsize=(7, 5))
    plt.plot(df_cupy["n"], df_cupy["flops"] / GIGA, label="CuPy Single Node")
    plt.plot(df_nums_1["n"], df_nums_1["flops"] / GIGA, label="NumS (1 V100)")
    plt.plot(df_nums_2["n"], df_nums_2["flops"] / GIGA, label="NumS (2 V100s)")
    plt.plot(df_nums_4["n"], df_nums_4["flops"] / GIGA, label="NumS (4 V100s)")
    plt.plot(df_nums_8["n"], df_nums_8["flops"] / GIGA, label="NumS (8 V100s)")

    plt.plot(df_nums_8["n"], [EMPIRICAL_ELEMENTWISE / GIGA for n in df_nums_8["n"]], label="Empirical Peak", linestyle="--")
    plt.plot(df_nums_8["n"], [EMPIRICAL_ELEMENTWISE * 2 / GIGA for n in df_nums_8["n"]], label="Empirical Peak 2X", linestyle="--")
    plt.plot(df_nums_8["n"], [EMPIRICAL_ELEMENTWISE * 4 / GIGA for n in df_nums_8["n"]], label="Empirical Peak 4X", linestyle="--")
    plt.plot(df_nums_8["n"], [EMPIRICAL_ELEMENTWISE * 8 / GIGA for n in df_nums_8["n"]], label="Empirical Peak 8X", linestyle="--")
    plt.title(r"NumS GPU GFLOPS of Elementwise Operations on Size $n$ Array")
    plt.xlabel(r"Array Size $n$ ($10^9$ scale)")
    plt.ylabel("GFLOPS")
    # plt.yscale("log")
    # plt.xscale("log")
    plt.legend()
